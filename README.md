...
to launch the system. Use the Xterm teleop keyboard to fly the drone to the center of the frame. If the system does not automatically start tracking, run `ros2 service call /reset-pose std_srvs/srv/Empty` to reset the tracked pose. Once the drone is being tracked, manually set the control\_yaw\_deg parameter until the drone model visualization is correctly oriented. run `ros2 topic pub -1 /acquire_drone std_msgs/msg/Empty`to hand over control to the automatic system. It is reccomended to use RQT to manage this startup.
